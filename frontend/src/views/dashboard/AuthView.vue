  ...
  <div class="auth-page" :class="[savedTheme, savedLang]">
    ...
    <div class="auth-container" :class="{ 'is-register': isRegister }">
      ...
      <div class="panel panel--white">
        ...
        <div class="form-slot form-slot--left" :class="{ 'form-slot--faded': !isForgot || formFading }" style="z-index:3">
          <div class="form-inner">
            <div class="brand">⬆⬇ UpDown</div>
            <h2>{{ authT.resetTitle }}</h2>
            <p class="subtitle">{{ authT.resetSub }}</p>

            <transition name="alert-slide">
              <div v-if="forgotError" class="alert alert--error">
                <span>{{ forgotError }}</span>
                <button class="alert-close" @click="forgotError = ''">×</button>
              </div>
            </transition>
            <transition name="alert-slide">
              <div v-if="forgotSuccess" class="alert alert--info">{{ forgotSuccess }}</div>
            </transition>

            <div v-if="forgotStep === 1">
              <div class="form-group">
                <input class="input" v-model="forgotForm.email" type="text" inputmode="email" :placeholder="authT.emailPlaceholder" @keyup.enter="sendForgotCode" />
              </div>
              <button class="btn-main" type="button" :disabled="forgotLoading" @click="sendForgotCode">
                {{ forgotLoading ? '...' : authT.sendResetCode }}
              </button>
            </div>

            <div v-if="forgotStep === 2">
              <div class="alert alert--info">{{ authT.codeSent }} {{ forgotForm.email }}</div>
              <div class="form-group">
                <input class="input" v-model="forgotForm.code" placeholder="123456" maxlength="6" @keyup.enter="doResetPassword" />
              </div>
              <div class="form-group">
                <div class="input-wrapper">
                  <input class="input input--with-icon" v-model="forgotForm.newPassword" :type="showForgotPass ? 'text' : 'password'" :placeholder="authT.newPasswordLabel" @keyup.enter="doResetPassword" />
                  <button class="eye-btn" type="button" @click="showForgotPass = !showForgotPass">
                    <svg v-if="!showForgotPass" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                    <svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"/><line x1="1" y1="1" x2="23" y2="23"/></svg>
                  </button>
                </div>
              </div>
              <button class="btn-main" type="button" :disabled="forgotLoading" @click="doResetPassword">
                {{ forgotLoading ? '...' : authT.setNewPassword }}
              </button>
            </div>

            <div class="form-options" style="margin-top:16px">
              <button class="link-btn" type="button" @click="backToLogin">← {{ authT.backToLogin }}</button>
            </div>
          </div>
        </div>

        <!-- Форма входа — левая половина -->
        <div class="form-slot form-slot--left" :class="{ 'form-slot--faded': isRegister || isForgot || formFading }">
          <div class="form-inner">
            <div class="brand">⬆⬇ UpDown</div>
            <h2>{{ authT.signIn }}</h2>
            ...
            <template v-if="loginMode === 'password'">
              ...
              <div class="form-group" v-show="!requires2FA">
                <input class="input" v-model="loginForm.email" type="text" inputmode="email" autocomplete="email" :placeholder="authT.emailPlaceholder" @keyup.enter="handleLogin" />
              </div>
              <div class="form-group" v-show="!requires2FA">
                <div class="input-wrapper">
                  <input
              ...
              <div v-show="requires2FA">
                ...
                <div class="form-group">
                  <input class="input" v-model="twoFaCode" placeholder="123456" maxlength="6" @keyup.enter="handleLogin" />
                </div>
              </div>
            </template>
            ...
            <template v-else>
              <div class="form-group" v-if="!loginCodeSent">
                <input class="input" v-model="loginForm.email" type="text" inputmode="email" autocomplete="email" :placeholder="authT.emailLabel" @keyup.enter="handleLogin" />
              </div>
              <div v-if="loginCodeSent">
                <div class="alert alert--info">{{ authT.codeSent }} {{ loginForm.email }}</div>
                <div class="form-group">
                  <input class="input" v-model="loginForm.code" placeholder="123456" maxlength="6" @keyup.enter="handleLogin" />
                </div>
              </div>
            </template>
            ...
            <div class="form-options">
              ...
              <button class="link-btn link-btn--right" type="button" @click="goToForgot" v-if="loginMode === 'password' && !requires2FA">
        ...
        <div class="form-slot form-slot--right" :class="{ 'form-slot--faded': !isRegister || formFading }">
          <div class="form-inner">
            ...
            <div v-if="regStep === 1">
              <div class="form-group">
                <input class="input" v-model="regForm.email" type="text" inputmode="email" autocomplete="email" :placeholder="authT.emailLabel" @keyup.enter="sendRegCode" />
              </div>
...
async function doResetPassword() {
  ...
  try {
    await authApi.resetPassword({ email: forgotForm.value.email, code: forgotForm.value.code, newPassword: forgotForm.value.newPassword })
    forgotSuccess.value = authT.value.resetSuccess
    setTimeout(() => backToLogin(), 2000)
  } catch (e) { forgotError.value = translateBackendError(e.response?.data?.message, authT.value.errExpiredCode) }
  finally { forgotLoading.value = false }
}
...
@media (max-width: 680px) {
  ...
  .brand { font-size: 14px; margin-bottom: 20px; }
  h2 { font-size: 26px; }
  .subtitle { margin-bottom: 22px; }
}

@media (max-width: 380px) {
  .panel--blue { padding: 28px 16px; }
  .form-slot { padding: 28px 16px 40px; }
  h2 { font-size: 22px; }
}
</style>